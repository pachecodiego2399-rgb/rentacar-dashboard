...
lib/airtable.ts          # Cliente de Airtable (server-only)
lib/types.ts              # Tipos compartidos (Auto, EstadoAuto, etc.)
app/api/autos/route.ts   # API route que expone los datos al frontend
...
- **AIRTABLE_API_KEY**: Personal Access Token generado en
  ...
  permiso de lectura (`data.records:read`) sobre la base del cliente.
...
Airtable solo se usa en `lib/airtable.ts`, importado exclusivamente desde la
API route (`app/api/autos/route.ts`), así que nunca llega al navegador.
